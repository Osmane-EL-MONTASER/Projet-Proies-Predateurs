...
public class Precipitation : MonoBehaviour {
    ...
    /// fonction Update().
    /// </summary>
    private static object _applyTextureLock = new object();
    ...
    void Update() {
        ...
        if(!_alreadyApplied && _timeAccumulator >= PrecipitationSpeed && Monitor.TryEnter(_applyTextureLock)) {
            XOrg += PrecipitationOffsetX;
            YOrg += PrecipitationOffsetY;

            _alreadyApplied = true;
            _timeAccumulator = 0f;
            Monitor.Exit(_applyTextureLock);
        }
    }
    ...
    private void calcNoise() {
        while(true) {
            if(_alreadyApplied && Monitor.TryEnter(_applyTextureLock)) {
